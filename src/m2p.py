...
def note_to_axis(note, max):
    """Map MIDI note number to x-position on the screen."""
    return p5.remap(note, (MIN_NOTE, MAX_NOTE), (0, max))

def note_to_color(note):
    """Map MIDI note number to a hue value for coloring."""
    hue = p5.remap(note, (MIN_NOTE, MAX_NOTE), (0, 360))
    return p5.Color(hue, 100, 100)  # Use the Color class
...
class Note:
    ...
    def __init__(self, msg):
        ...
        # self.y = HEIGHT - note_to_axis(self.note, HEIGHT)
        self.y = HEIGHT / 2
        self.size = p5.remap(self.velocity, (0, 127), (10, 50 * CIRCLE_SCALE))
    ...
    def update(self):
        ...
        if self.active:
            ...
            self.size -= CIRCLE_SCALE  # Shrink after note ends
            if self.size <= 0:
                self.finished = True
