...
func get_category_name(category: int) -> String:
	return Globals.InstrumentCategory.keys()[category]
...
func _on_arlez_midi_player_midi_event(channel: MidiPlayerAddon.GodotMIDIPlayerChannelStatus, event: Variant) -> void:
	match event.type:
		SMF.MIDIEventType.program_change:
			var category = Globals.InstrumentCategory.PERCUSSIVE if channel.number == 9 else get_instrument_category(event.number)
			print("PROGRAM CHANGE, %s -> %s" % [channel.instrument_name, get_category_name(category)])
