...
var target_dancer_position: Vector3
var target_dancer_look_at: Vector3
var target_dancer_height: float
...
func update_dancer(delta):
	var dancer = $DancingSphere
	dancer.position = lerp(dancer.position, target_dancer_position, 0.5 * delta)

	# dancer.look_at(lerp(dancer.position, target_dancer_look_at, 0.1 * delta))
	dancer.rotation = dancer.rotation.slerp(target_dancer_look_at, 0.5 * delta)
	dancer.scale = lerp(dancer.scale, Vector3(1, 1, target_dancer_height), 0.5 * delta)

func update_bg(delta):
	world_env.environment.background_color = lerp_between_colors(world_env.environment.background_color, target_bg_color, 1.0 * delta)
...
func color_from_notes(avg_pitch: float, sum_velocity: float, max_velocity: float, min_saturation: float, max_saturation: float, min_brightness: float, max_brightness: float) -> Color:
	...
	print(avg_pitch, " ", hue, " ", sat, " ",brightness)
...
func update_active_notes():
	...
	print(instrument_stats["total"][VELOCITY])
	...
	target_bg_color = (color_from_notes(avg_pitch, clamped_sv, MAX_VELOCITY, 0.2, 0.2, 0.5, 0.5))
	var rng = RandomNumberGenerator.new()
	rng.randomize()
	var dancer_x = 0.0
	var dancer_y = remap(avg_pitch, 0, 127, -1, 1)
	var dancer_z = remap(clamped_sv, 0, MAX_VELOCITY, -4, -0.5)
	target_dancer_position = Vector3(dancer_x, dancer_y, dancer_z)
	target_dancer_look_at = target_dancer_position.normalized()
	target_dancer_height = remap(avg_pitch, 0, 127, -1, 2)
